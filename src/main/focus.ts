...
function runPowerShell(script: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code) => {
      ...
      else reject(new Error(Buffer.concat(stderr).toString('utf8').trim() || `PowerShell exited ${code}`))
    })
  })
}
 ...
 */
export async function focusSessionWindow(session: SessionState): Promise<SessionActionResult> {
